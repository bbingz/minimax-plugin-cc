...
**状态**：草稿 v4，两轮 spec review + 一轮 plan review 反向回炉 spec（见附录 B）
...
- 不做实时日志事件流 UX（依赖 P0.3 增量 flush 结论，失败即 v0.2 补）
- 不做 per-command 切模型（依赖 P0.7 workspace-local config 结论，失败即 v0.2 补）
...
| 大 prompt | `-p ""` + stdin | **P0.4 probe 决定**：stdin pipe / tmpfile / argv 分块 | Phase 0 硬门 |
...
Mini-Agent 启动时打印一行 `📝 Log file: /Users/<user>/.mini-agent/log/agent_run_<ts>.log`。文件结构（实测样本）：
...
{ ... 完整的 Anthropic message JSON，包含 messages[] 和 tools[] ... }
...
{ ... assistant 响应 JSON，Anthropic message 格式，包含 content[] 数组 ... }
```

**解析策略 v3（codex plan review CRITICAL 修正：按分隔符 split 会错放 header 和 JSON；改为线性扫描状态机）**：
...
```
input: log file text
output: array of { n, kind: "REQUEST"|"RESPONSE", json: parsed | null, raw }
...
for line in lines:
  if state == SEEK_HEADER:
    match /^\[(\d+)\] (REQUEST|RESPONSE)$/ → state = SKIP_TO_BODY; current = {n, kind}
  elif state == SKIP_TO_BODY:
    // 跳过 header 行后的 Timestamp 行 / 分隔符 / 空行
    // 进入 body 的标志：遇到第一个以 `{` 起首的行（允许前导空白）
    ...
    braceDepth += countOpenBraces(line) - countCloseBraces(line)
    if braceDepth <= 0:
      // JSON body 结束（允许 depth < 0 容错，但 parse 时再检）
      current.raw = accumulator.join("\n")
      current.json = tryParse(current.raw)
      blocks.push(current); state = SEEK_HEADER

// 循环结束：若仍在 COLLECT_BODY，当前 block 不完整
if state == COLLECT_BODY:
  current.raw = accumulator.join("\n")
  current.json = null  // 明确 unparseable
  current.truncated = true
  blocks.push(current)
```

`countOpenBraces` / `countCloseBraces` 忽略字符串内部的 `{` / `}`：用状态机跳过 `"..."` 区域（处理 `\"` / `\\` 转义）。实现约 40 行代码。

**选终态 block**（逻辑不变）：

1. 过滤得 `responseBlocks = blocks.filter(b => b.kind === "RESPONSE")`
2. 从 `responseBlocks[responseBlocks.length - 1]` 倒序扫，选第一个满足：
   - `b.json` 非 null（parse 成功）
   - **且** (`b.json.stop_reason` 是合法终态值 ∈ {`end_turn`, `stop_sequence`, `tool_use`, `max_tokens`}  **或**  `b.json.content[]` 中有非空 `{type:"text", text}`)
3. 命中 → 抽 `content[]`：`type:"text"` → text 拼接；`type:"tool_use"` → toolCalls 数组
...
**第二层 · 日志结构**（strip ANSI 后解析；优先于 stdout sentinel）：

| 日志状态 | 判定 |
|---|---|
| 有终态 RESPONSE block（§3.5），`stop_reason ∈ {end_turn, stop_sequence}` + 非空 text | `success` |
| 有终态 RESPONSE block，`stop_reason = max_tokens` | `success-but-truncated` |
| 有终态 RESPONSE block，`stop_reason = tool_use`（agent 内部未闭环） | `incomplete`（§4.5 错误呈现） |
| 无任何 RESPONSE block | 回落到第三层 |
