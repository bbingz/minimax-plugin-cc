...
export function renderSetupReport(report) {
  ...
  lines.push(
    `| MiniMax CLI | ${report.minimax.available ? "OK" : "Missing"} | ${report.minimax.detail} |`
  );
  lines.push(
...
export function renderJobSubmitted(job) {
  return `**Job submitted:** \`${job.jobId}\` (PID: ${job.pid})\n\nUse \`/minimax:status ${job.jobId}\` to check progress, \`/minimax:result ${job.jobId}\` to get output.\n`;
}
...
function renderFollowUpHints(job) {
  ...
  if (job.status === "running" || job.status === "queued") {
    hints.push(`  - Cancel: \`/minimax:cancel ${job.id}\``);
  }
  if (job.status === "completed" || job.status === "failed") {
    hints.push(`  - View result: \`/minimax:result ${job.id}\``);
  }
  if (job.kind === "task" && job.write) {
    hints.push(`  - Review changes: \`/minimax:review --wait\``);
  }
  if (job.geminiSessionId) {
    hints.push(`  - Resume: \`/minimax:rescue --resume-last\``);
  }
  return hints.join("\n");
}
